...
| Claude Code | High | Install the full folder as `.claude/skills/carrel` or `~/.claude/skills/carrel`; standalone (without the Carrel plugin) it uses this bundled runtime like any other host. The Carrel Claude Code plugin is separate: its commands and hooks call the installed typed `carrel` CLI, not this pack. |
...
| Claude Desktop (chat) | Partial | Skills load only via cloud ZIP upload (Customize -> Skills); no local `SKILL.md` folder discovery. Local vault access requires connected folders, desktop extensions, or local MCP. |
| Claude Cowork | High with a connected local folder and Claude Desktop open; remote-only sessions cannot reach a local vault | Cowork runs Claude Code under the hood, so wherever Claude Desktop's Claude Code works with a connected local folder, Cowork works too. Scheduled or remote-only sessions with no connected folder (or with Desktop closed) cannot reach a local vault. |
