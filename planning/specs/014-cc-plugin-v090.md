...
|---|---|---|---|
| `carrel vault feedback export --redact-list <path>` | `/carrel-feedback` (file write) | Deterministic anonymization via explicit redact list; sweeps `friction-log/`, `capability-log/`, AND `reflections/` (read/write symmetry with `session-reflection` skill); writes flat-file `_meta/feedback-digest-<YYYY-MM-DD>.md` | Conversation guidance |
...
| `carrel vault mirror --write --from-stdin` | `/carrel-mirror` (file persistence) | **Monthly-filename** emission at `_meta/mirror/<YYYY-MM>.md` (same-month re-runs update; matches research-partner skill idempotency contract) | Mirror synthesis prose |
| `carrel vault reflect-log --append --from-stdin` | `/carrel-reflect` (file persistence) | Atomic append to `_meta/reflections/reflection-<YYYY-MM-DD>.md`; seeds from vault's `_templates/reflection.md` on first write of day if present | Reflection prompts + framing |
...
| `carrel batch convert <folder>` / `carrel batch transcribe <folder>` (both accept `--unattended`; transcribe also accepts `--kind interview\|meeting\|lecture\|recording`) | `/carrel-batch` (folder enumeration) | Folder walk + per-file CLI dispatch; `--unattended` writes `_meta/pending-decisions.md` instead of asking (deduplicates by exact row match on re-runs). Audio extensions: `.m4a .mp3 .wav .mp4 .webm .mov .ogg .flac`. | Pre-batch confirmation prose; UNATTENDED-mode contract narration |
| `carrel automate configure` (typed flags only) | `/carrel-automate` (file writes) | Calls `policy.trust.is_allowed("automation:propose")` internally as gate. Surface: `--enabled`, `--trust-level`, `--schedule`, `--review-cadence`, `--model` (required/typed) plus 7 capability booleans (`--inbox-processing`, `--vault-health`, `--cross-linking`, `--gap-analysis`, `--draft-feedback`, `--reflection-synthesis`, `--wiki-maintenance`) that preserve existing profile values when not passed. Writes `AutomationConfig` to `environment.json`. | 10-step interview flow; trust-level explainer; Desktop App walkthrough |
